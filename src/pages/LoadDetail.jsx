...
export default function LoadDetail() {
  ...
  useEffect(() => {
    fetch(`/api/loads/${id}`).then(r => r.json()).then(l => {
      setLoad(l)
      setForm({
        barco: l.barco?._id || '',
  ...
  const submit = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        setSnack({ open: true, message: err.error || 'Error actualizando carga', type: 'error' })
        return
      }
      ...
      setSnack({ open: true, message: 'Error de red actualizando carga', type: 'error' })
    }
  }
  ...
  return (
    <section className="card">
      ...
      <div style={{ padding: 16 }}>
        ...
        <p><strong>Fecha de descarga:</strong> {formatDateLabel(load.fecha_de_descarga)}{load.hora_de_descarga ? `, ${load.hora_de_descarga}` : ''}</p>
        <p><strong>Cash:</strong> {load.cash ? 'Sí' : 'No'}</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 12 }}>
          <div className="card">
            ...
            <div style={{ padding: 12 }}>
              <p><strong>Total palets:</strong> {totalPallets}</p>
              {Object.keys(tipoCounts).length > 0 ? (
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 6 }}>
                  {Object.entries(tipoCounts).map(([tipo, count]) => (
                    <span key={tipo} style={{ background: '#eef2f7', borderRadius: 6, padding: '4px 8px' }}>
                      {tipo}: {count}
                    </span>
                  ))}
                </div>
          ...
          <div className="card">
            ...
            <div style={{ padding: 12 }}>
              {palletsInLoad.length === 0 ? (
                <div style={{ color: 'var(--text-secondary)' }}>Sin palets asociados</div>
              ) : (
                palletsInLoad.map(p => (
                  <div
                    key={p._id}
      ...
      <Modal open={open} title="Modificar carga" onClose={() => setOpen(false)} onSubmit={submit} submitLabel="Guardar" width={640} bodyStyle={{ gridTemplateColumns: '1fr' }}>
      ...
      <Snackbar open={snack.open} message={snack.message} type={snack.type} onClose={() => setSnack({ ...snack, open: false })} />
    </section>
  )
}