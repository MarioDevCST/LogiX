...
export default function ShipDetail() {
  ...
  const submit = async () => {
    try {
      const res = await fetch(`/api/ships/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, modificado_por: (getCurrentUser()?.name || 'Testing') }) })
      if (!res.ok) {
