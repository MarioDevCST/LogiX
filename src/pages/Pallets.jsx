...
export default function Pallets() {
  ...
  const latestPalletsPage = useMemo(() => {
    const arr = [...palletDocs].sort(
      (a, b) =>
        new Date(b.createdAt || b.fecha_creacion) -
        new Date(a.createdAt || a.fecha_creacion)
    );
        ...
        >
                  ...
                  >
                    ...
                    >
                      ...
                      {Array.isArray(l.palets) && l.palets.length > 0 && (
                        ...
                        >
                          Palets:{" "}
                          {l.palets.map((p) => p.numero_palet).join(", ")}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="card">
            ...
            <div style={{ padding: 12 }}>
              ...
              <Pagination
                page={palletsListPage}
                pageSize={palletsListPageSize}
                total={palletDocs.length}
                onPageChange={(p) => setPalletsListPage(Math.max(1, p))}
                onPageSizeChange={(s) => {
                  setPalletsListPageSize(s);
                  setPalletsListPage(1);
                }}
              />
            </div>
          </div>
