...
const PalletSchema = new mongoose.Schema(
  {
    ...
    tipo: {
      type: String,
      enum: ["Seco", "Refrigerado", "Congelado", "Técnico"],
      required: true,
    },
    base: { type: String, enum: ["Europeo", "Americano"], default: "Europeo" },
    carga: {
...
PalletSchema.pre("findOneAndUpdate", async function (next) {
  try {
    ...
    if (numero || carga) {
      ...
      const cargaLabel =
        load?.nombre || (load?.barco ? String(load.barco) : "");
      update.$set = {
        ...(update.$set || {}),
        nombre: `${numeroFinal} - ${cargaLabel || "Sin carga"}`,
      };
