...
function getMonthMatrix(date) {
  ...
  const firstDay = (start.getDay() + 6) % 7; // 0-6 (lunes = 0)
...
function getWeekDays(date) {
  ...
  start.setDate(d.getDate() - ((d.getDay() + 6) % 7)); // lunes
...
function getFortnightDays(date) {
  const d = new Date(date);
  const start = new Date(d);
  start.setDate(d.getDate() - ((d.getDay() + 6) % 7)); // lunes
  start.setHours(0, 0, 0, 0);
...
export default function Calendar({
  ...
  onItemClick,
  loading = false,
  dateKey = "fecha_de_carga",
  secondaryDateKey = "fecha_de_descarga",
  statusKey = "estado_viaje",
}) {
  const cells = React.useMemo(() => {
    if (mode === "week") return getWeekDays(month);
    if (mode === "fortnight") return getFortnightDays(month);
    return getMonthMatrix(month);
  }, [month, mode]);
  ...
  const label =
    mode === "week" || mode === "fortnight"
      ? (() => {
          ...
          return `${mode === "week" ? "Semana" : "Quincena"} · ${fmtStart} – ${fmtEnd}`;
        })()
  ...
  return (
    <section className="calendar">
      ...
      <div className="calendar-grid">
        {weekday.map((w) => (
          <div key={`w-${w}`} className="calendar-cell calendar-weekday">
