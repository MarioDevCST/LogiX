...
import { ROLES, ROLE_LABELS, getCurrentRole, getCurrentUser } from "../utils/roles.js";
import {
  deleteMessageById,
  fetchMessageById,
  updateMessageById,
...
export default function MessageDetail() {
  ...
  const toggleRoleEdit = (role) => {
    ...
    setEditForm({
      ...editForm,
      roles: has ? editForm.roles.filter((r) => r !== role) : [...editForm.roles, role],
    });
  };
      ...
      <section className="card">
        ...
        <div style={{ padding: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", gap: 8 }}>
            ...
            <div>
              {Array.isArray(message.roles) && message.roles.length > 0
                ? message.roles.map((r) => ROLE_LABELS[r] || r).join(", ")
                : "Todos"}
            </div>
          </div>
        </div>
